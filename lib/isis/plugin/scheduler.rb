  ...
  module Plugin
    class Scheduler < Isis::Plugin::Base
      ...
      def load_from_config
        config_file = File.expand_path(File.join('config', 'scheduler.yml'))
        raise Isis::Plugin::PluginSetupError, 'config/scheduler.yml config file not found' unless File.exists?(config_file)
